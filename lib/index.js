    ...
    const scheme = {
        authenticate: function (request, reply) {
            ...
            if (!username && !settings.allowEmptyUsername) {
                return reply(Boom.unauthorized('HTTP authentication header missing api key (as username)', 'Basic', settings.unauthorizedAttributes));
            }
            ...
            settings.validateFunc(request, username, password, (err, isValid, credentials) => {
                ...
                if (!isValid) {
                    return reply(Boom.unauthorized('Invalid api key', 'Basic', settings.unauthorizedAttributes), null, { credentials: credentials });
                }
